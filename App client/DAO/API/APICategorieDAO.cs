...
namespace DAO.API
{
    public class APICategorieDAO : ICategorieDAO
    {
        ...
        public async Task<Categorie[]> GetByIdAsync(IEnumerable<int> no)
        {
            ...
            var status = JsonConvert.DeserializeObject<Response<Categorie>>(await response.Content.ReadAsStringAsync());
            if (status.success)
                return status.values.Length == no.Count() ? status.values : throw new DAOException("An entry is missing", DAOException.ErrorCode.MISSING_ENTRY);
            else
            {
                var err = status.errors.First();
        ...
        public async Task<Categorie[]> GetFilteredAsync(int maxCount, int page, string? orderBy = null, bool reverseOrder = false, string? search = null)
        {
            var obj = new Dictionary<string, object>();
            var filters = new Dictionary<string, object>();
            obj.Add("filters", filters);
        ...
        public async Task<Categorie[]> UpdateAsync(IEnumerable<(Categorie, Categorie)> values)
        {
            if (values == null)
                throw new ArgumentNullException(nameof(values));
            var obj = new
            ...
            var jsonObj = JsonConvert.SerializeObject(obj, Formatting.None);
            var url = new Uri("categorie/EditCategorie.php", UriKind.Relative);
            var response = await Client.PostAsync(url, new StringContent(jsonObj, Encoding.UTF8, "application/json"));
            var status = JsonConvert.DeserializeObject<Response<Categorie>>(await response.Content.ReadAsStringAsync());
            if (status.success)
