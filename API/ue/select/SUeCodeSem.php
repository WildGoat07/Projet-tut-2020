...
if ($db) {
    $requete = $db->query("SELECT code_ue, libelle_ue, nature, ECTS, code_ue_pere, code_sem FROM ue WHERE code_sem LIKE '%" .$_POST['code_sem']. "%'
                                                                                                        or code_sem LIKE UPPER('%" .$_POST['code_sem']. "%')");
